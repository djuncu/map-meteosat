...
# homemade tools
from . import xr_read_sat_files
from . import utils_xr_process
...
#def read_one_file_msg_with_slice_to_2D_array(f_in_path, varname, scaling, offset=0., valid_range = [], missing=[], slice_first_dim=slice(None,None), slice_second_dim=slice(None,None)):
#    """
#    Read MSG data with h5py
#    Parameters : 
#        varname : string. Corresponds to the name of the variable to read in the hdf5 files.
...
#    file = h5py.File(f,'r')
#        
#    var = file[varname][slice_first_dim, slice_second_dim]
#    da = xr.DataArray(var, dims = ('y','x'))
#    del var
#    da = utils_xr_process.apply_scaling_offset_missing(da, scaling=scaling,offset=offset, valid_range = valid_range, missing=missing)
#    da = da.assign_attrs(file.attrs)
...
def read_one_file_msg_with_slice_or_indices_lists_to_1D_array(f_in_path, varname, scaling, offset=0., valid_range = [], missing=[], ilon_list=None, ilat_list=None, slice_first_dim=slice(None,None), slice_second_dim=slice(None,None)):
    """
    Read MSG data with h5py
    Parameters : 
        varname : string. Corresponds to the name of the variable to read in the hdf5 files.
    ...
    f = f_list[0]
    file = h5py.File(f,'r')
    if ilon_list is not None or ilat_list is not None:
        var_big = file[varname][:] # h5py indexing is less flexible than numpy array
        var = var_big[ilat_list, ilon_list] # we have to apply the slicing by indexes on the output of h5 reader, not directly when reading
    else:
        var = file[varname][slice_first_dim, slice_second_dim].ravel()
    da = xr.DataArray(var, dims = ('points'))
    del var
    da = utils_xr_process.apply_scaling_offset_missing(da, scaling=scaling,offset=offset, valid_range = valid_range, missing=missing)
    da = da.assign_attrs(file.attrs)
    file.close()
...
def read_one_file_msg_v3_to_1D(f_in_path, varnames, dtypes_dic, keep_attrs, store_read_to_netcdf=False, f_out=None, ilon_list=None, ilat_list=None, slice_first_dim=slice(None,None), slice_second_dim=slice(None,None)):
    """
    ...
    for i,varname in enumerate(varnames):       
        ...
        if ilon_list is not None or ilat_list is not None:
            ...
            var = file[varname][slice_first_dim, slice_second_dim].ravel()
        if dtypes_dic[varname] is None: da = xr.DataArray(var, dims = ('points'),name=varname)
        if dtypes_dic[varname] is not None: da = xr.DataArray(var, dims = ('points'),name=varname).astype(dtypes_dic[varname])
        del var
        if keep_attrs: 
...
def gather_on_sub_f_list_1D(sub_f_list, store_read_to_netcdf,f_out_sub, ilon_list=None, ilat_list=None, slice_first_dim=slice(None,None), slice_second_dim=slice(None,None), **kw_reader):
    first_t = None
    for i,f in enumerate(sub_f_list):
        print(f)
        ds_one = read_one_file_msg_v3_to_1D(f_in_path=f, **kw_reader, store_read_to_netcdf=False, f_out=None, ilon_list=ilon_list, ilat_list=ilat_list, slice_first_dim=slice_first_dim, slice_second_dim=slice_second_dim)
...
def read_and_stack_several_files_msg_v4_1D (f_in_path, varnames, dtypes_dic, keep_attrs = False, store_read_to_netcdf=False, f_out=None, chunk_f_list = 20, ilon_list=None, ilat_list=None, slice_first_dim=slice(None,None), slice_second_dim=slice(None,None)):
    """
    Read MSG data with h5py - no rescaling/filtering
    Parameters : 
        varnames : list of strings. Corresponds to the names of the variables to read in the hdf5 files.
    ...
    for it, sub_f_list in enumerate(sub_f_lists):
        ...
        my_utils_plot.ensure_dir(f_out_sub)
        gather_on_sub_f_list_1D(sub_f_list, store_read_to_netcdf=True, **kw_reader, f_out_sub =f_out_sub, ilon_list=ilon_list, ilat_list=ilat_list, slice_first_dim=slice_first_dim, slice_second_dim=slice_second_dim)
...
def read_and_stack_several_files_msg_no_xr_1D(f_in_path, sat_key, varnames, f_out_scaled, ilon_list=None, ilat_list=None, slice_first_dim=slice(None,None), slice_second_dim=slice(None,None)):
    d = read_input_dtypes(sat_key)
    dtypes_in = {k:v for k,v in d.items() if k in varnames}
    d = read_scaled_dtypes(sat_key)
    dtypes_sc = {k:v for k,v in d.items() if k in varnames}
    store_read_to_netcdf=True
    chunk_f_list=None # each nth file, store all to netcdf cache and reload it to remove data from memory
    if store_read_to_netcdf==False:
        f_out=None
        ds_all = read_and_stack_several_files_msg_v4_1D(f_in_path=f_in_path, varnames=varnames, dtypes_dic=dtypes_in, store_read_to_netcdf=store_read_to_netcdf, 
                                                     f_out=f_out, keep_attrs=True, chunk_f_list=chunk_f_list, ilon_list=ilon_list, ilat_list=ilat_list, slice_first_dim=slice_first_dim, slice_second_dim=slice_second_dim)
    else:
        homedir = os.path.expanduser("~")
        ...
        read_and_stack_several_files_msg_v4_1D(f_in_path=f_in_path, varnames=varnames, dtypes_dic=dtypes_in, store_read_to_netcdf=store_read_to_netcdf,
                                            f_out=f_out, keep_attrs=False, chunk_f_list=chunk_f_list, ilon_list=ilon_list, ilat_list=ilat_list, slice_first_dim=slice_first_dim, slice_second_dim=slice_second_dim)
        ds_all = xr.open_dataset(f_out)
        os.remove(f_out)
        
    ds_scaled = ds_all.copy()
    for varname in ds_all.data_vars:
        d = read_scaling_offset_fillvalue(sat_kw=sat_key, varname=varname)
