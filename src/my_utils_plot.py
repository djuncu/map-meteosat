...
def resampleMesh(lonval, latval, da_to_plot, plotXstep=None, plotYstep=None, da_ocean=[], oceanVal=-1):
    ...
    lonval = lonval[::plotYstep, ::plotXstep] # right order?
    latval = latval[::plotYstep, ::plotXstep]  

    lon_resamp = np.zeros([lonval.shape[0]-1,lonval.shape[1]-1])
    lat_resamp = np.zeros([lonval.shape[0]-1,lonval.shape[1]-1])
    
    v_resamp = np.empty([lonval.shape[0]-2,lonval.shape[1]-2])
    #ocean_resamp  = np.zeros([lonval.shape[0]-2,lonval.shape[1]-2])
    for i in range(lonval.shape[0]-1):
        for j in range(lonval.shape[1]-1):
            lon_resamp[i,j] = np.mean(lonval[i:i+2,j:j+2])
            lat_resamp[i,j] = np.mean(latval[i:i+2,j:j+2])
            
            if j != lonval.shape[1]-2 and i != lonval.shape[0]-2:
